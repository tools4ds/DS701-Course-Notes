...
Let $G$ be a connected graph with $n$ nodes. Let $d(i,j)$ be the shortest path distance between node $i$ and node $j$ in $G$.   
...
- **Perron-Frobenius Theorem**: For a connected graph, the largest eigenvalue is positive, and the corresponding eigenvector has all positive entries, ensuring meaningful centrality scores.
...
This tells us wwhen nodes have values in $\mathbf{x}$, how much __adjacent__ nodes vary.
...
We already know how to solve this constrained minimization problem.
...
So ... we are connecting functions on the graph $G$ with eigenvectors of the matrix $L$. This is quite remarkable.
...
Another way of saying that $\mathbf{x} \perp {\mathbf 1}$ is that the entries of $\mathbf{x}$ sum to 0.
...
Recall that the energy in a stretched spring is proportional to the square of its stretched length.  
...
This plot shows that many clusters are well-separated in this space, but some are still overlap.
