...
* $\alpha_u$ is the $n\times 1$ per-user offset of user $u$, 
* $\beta_i$ is the $m\times 1$ per-item offset of item $i$, and
...
the set of users who have rated item $i$.
...
approaches to building recommender systems are use Deep Learning.
...
dense vectors and passed to a final MLP.
