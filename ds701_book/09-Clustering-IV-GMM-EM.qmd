...
- Use the Expectation Maximization (EM) algorithm to iteratively determine the parameters of our the Gaussian clusters
...
The likelihood function $L(\mathbf{\theta}, x)$ represents the probability
of observing the given data $x$ as a function of the parameters $\mathbf{\theta}$ of the distribution.
...
= \prod_{n=1}^{N}(x_n\vert \mu, \sigma^{2}) \
...
$$
f(\mathbf{x}\vert \bar{\mu}, \Sigma) = \frac{1}{(2\pi)^{d/2}\vert \Sigma \vert^{1/2}}e^{-\frac{1}{2}(\mathbf{x}-\mathbf{\mu})^T\Sigma^{-1}(\mathbf{x}-\mathbf{\mu})},
$$

where $\Sigma\in\mathbb{R}^{d\times d}$ is the covariance matrix, $\vert \Sigma\vert$, denote the determinant of $\Sigma$, and $\mathbf{\mu}$ is the $d$-dimensional vector of expected values.
...
The notation $\mathbf{x} \sim \mathcal{N}(\mathbf{\mu}, \Sigma)$ is used to denote a _multivariate_
random variable that is normally distributed with mean $\bar{\mathbf{\mu}}$ and covariance matrix $\Sigma$.
...
We assume that each cluster is distributed normally, according to some (unknown) parameters $\mathbf{\mu}_i, \Sigma_i$. We also assume that the probability that a point belongs to a particular cluster is given by $w_i$.

A Gaussian mixture model is defined by these parameters $w_i, \mathbf{\mu}_i, \Sigma_i$ for $i=1, \ldots, K$. 

We can use these parameters to compute the probability that a point $\mathbf{x}$ belongs to a particular cluster $C_k$.

Similar to MLE, we must compute the parameters $w_i, \mathbf{\mu}_i, \Sigma_i$.
...
* cluster mean $\mathbf{\mu}_i$, and 
* cluster covariance  $\Sigma_i$

for each $i=1, \ldots, K$?

:::: {.fragment}
We will formulate a likelihood function for these parameters $\mathbf{\theta}_i = (w_i, \mathbf{\mu}_i, \Sigma_i)$ and then optimize this function.
::::
...
Let's gather for all $i=1, \ldots, K$, the coefficients $w_i,\mathbf{\mu}_i, \Sigma_i$ into a vector $\mathbf{\theta}$.

The likelihood function for a Gaussian mixture model with $N$ data points

$$
L(\mathbf{\theta}, x_1, \ldots, x_n) = \prod_{n=1}^{N}\sum_{k=1}^{K}w_k \frac{1}{(2\pi)^{d/2}\vert \Sigma_k \vert^{1/2}}e^{-\frac{1}{2}(\mathbf{x}_n-\mathbf{\mu}_k)^T\Sigma_{k}^{-1}(\mathbf{x}_n-\mathbf{\mu}_{k})}
$$

The log-likelihood is

$$
\log{\left(L(\mathbf{\theta}, x_1, \ldots, x_n)\right)} = \sum_{n=1}^{N}\log{\left(\sum_{k=1}^{K}w_k \frac{1}{(2\pi)^{d/2}\vert \Sigma_k \vert^{1/2}}e^{-\frac{1}{2}(\mathbf{x}_n-\mathbf{\mu}_k)^T\Sigma_{k}^{-1}(\mathbf{x}_n-\mathbf{\mu}_{k})}\right)}.
$$

Unlike the log of a product, the log of a sum does not simplify nicely. As a result, the partial derivatives with respect to $\mathbf{\mu}_k$, depend on the covariances and mixture weights (similarly for the other partial derivatives of $\Sigma_k$ and $w_k$).
...
Initialize the parameters $w_k, \mathbf{\mu}_k, \Sigma_k$ for $k=1, \dots, K$. The final result will be sensitive to this choice, so a good (and fast) initialization procedure is $k$-means.

__Step 2 Expectation__ 

Use the current values for $w_k, \mathbf{\mu}_k, \Sigma_k$ and for each of the $N$ data points $x_n$, compute the posterior probabilities

$$
r_{nk} = \frac{w_k f(\mathbf{x}_n \vert \mathbf{\mu}_{k}, \Sigma_k)}{\sum_{i=1}^{K}w_i f(\mathbf{x}_n \vert \mathbf{\mu}_{j}, \Sigma_j)},
$$

where $f(\mathbf{x}_n \vert \mathbf{\mu}_{k}, \Sigma_k)$ is the multivariate Gaussian.

__Step 3 Maximization__ 

Using the values $r_{nk}$ for $n=1,\ldots, N$ and $k=1, \ldots, K$. First compute $N_{k} = \sum_{n=1}^{N} r_{nk}$. Then compute updated $w_k, \mathbf{\mu}_k, \Sigma_k$ according to the formulas

$$
\mathbf{\mu}_{k}= \frac{1}{N_k}\sum_{n=1}^{N} r_{nk} \mathbf{x}_n, \quad 
\Sigma_{k} = \frac{1}{N_k}\sum_{n=1}^{N} (\mathbf{x}_n \mathbf{\mu}_k)^{T}(\mathbf{x}_n -\mathbf{\mu}_k), \quad
w_k = \frac{N_k}{N}.
$$
...
- There are $K$ $d$-dimensional cluster centers $\mathbf{\mu}_k$
...
- Updating $\mathbf{\mu}_k$ requires $N$ vector summations
