...
![](figs/Sir_Francis_Galton_by_Charles_Wellington_Furse.jpg){width=50%}
...
![](figs/galton-title.png){width=55%}

In 1886 Francis Galton published his observations about how random factors affect outliers.

This notion has come to be called "regression to the mean" because unusually large
or small phenomena, after the influence of random events, become closer to their
mean values (less extreme).

![](figs/galton-regression.png){width=55%}
...
It is a form of supervised learning: whereas __classification__ deals with
predicting categorical features (labels or classes), __regression__ deals with
predicting continuous features (real values).
...
The independent variables are collected into a matrix $X,$ which is called the
__design matrix.__
...
# Annotate points
for i in range(len(x)):
    ax.text(x[i], y[i] + 0.2, f'({x[i]}, {y[i]})', fontsize=9, ha='center')
    ax.text(x[i], y_line[i] - 0.3, f'({x[i]}, {y_line[i]:.1f})', fontsize=9, ha='center')
...
![](figs/Lay-fig-6-6-1.jpg){width=55%}
...
$\beta$ that minimizes the sum of squared residuals, ie,
...
length of $\epsilon$, ie, $\Vert\epsilon\Vert.$
...
$(u_1, v_1, y_1), \dots, (u_n, v_n, y_n)$ where $u_j, v_j$, and $y_j$ are
...
We can see that there the general principle is the same across all the different
...
Thus, for any linear model where $X^TX$ is invertible, the least squares estimate
$\hat{\mathbf{\beta}}$ is given by $(X^TX)^{-1}X^T\mathbf{y}$.
...
Then it turns out that the total sum of squares is exactly equal to the sum of
...
However, some of the independent variables may not contribute to the accuracy of
the prediction. 
