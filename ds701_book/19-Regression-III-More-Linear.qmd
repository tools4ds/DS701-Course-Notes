...
For any given $c$ this has a closed-form solution in which $\hat{\boldsymbol{\beta}} = (X^TX +cI)^{−1}X^T$ is a linear function of $X$ (just as in ordinary least squares).
...
Patsy is the name of the interpreter that parses the formulas in the `smf` submodule. These formulas are R-styled formulas. It is based off this [package](https://patsy.readthedocs.io/en/latest/) used to describe statistical models in Python.
