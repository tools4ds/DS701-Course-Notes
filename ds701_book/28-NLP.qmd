...
Word embeddings represent word as dense vectors in high-dimensional spaces.
...
:::: {.columns}
::: {.column width="70%"}
- This is achieved by adding a vector $\mathbf{t}^{(i)}$ to the word embedding To establish the order of the words, we use positional encodings. 
...
Attention allows a language model to understand correctly that *it* refer to the elephant and not the river.
...
:::: {.columns}
::: {.column width="60%"}
To compute the query, key, and value vectors, compute
Each $\mathbf{q}^{(i)}$, $\mathbf{k}^{(i)}$, and $\mathbf{v}^{(i)}$ is associated with $\mathbf{x}^{(i)}$.
$$
\begin{align*}
...
:::: {.columns}
::: {.column width="60%"}
- Training with features on different scales takes longer.
- The potential cause of exploding gradients.
...
1. Attends to the input of the encoder with *cross attention*.
...
The softmax layer outputs the probability of a potential word being generated. The word with the maximum probability is chosen.
...
### Bidirection Encoder Representations from Transformers
