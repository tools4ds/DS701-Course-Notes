...
![](figs/NN-figs/neuron_model.jpeg){width="75%" .lightbox}
...
linear model and loses all its expressive power. To dive deeper, see Chapter 3
of [Understanding Deep Learning](https://udlbook.github.io).
:::
...
![](figs/NN-figs/L24-fcn-dag.png){width="75%" .lightbox}
...
The example above shows a network with _3 inputs_, three hidden layers of 4, 2 and 3 neurons, respectively, followed by one layer that produces two continuous value outputs. 
...
To do that we will build a data wrapper as a `class` called `Value` and gradually
...
To do that, we'll need to store pointers to the operands of each operation.

To start with, we'll accept a tuple of child nodes in the initializer and store
that as a set in the object.
...
::: {.callout-important}
We needed the node values in order to update the gradients. We calculate all
the node values in the **forward pass**.
:::
...
Let's look at the effect `b` has on `L`.
