...
if os.path.exists(aapl_file_path):
    # Load data from file
    aapl_data = pd.read_csv(aapl_file_path, index_col=0, parse_dates=True)
...
manipulation using the `datetime` library and the `pandas` library which is
a summary of the [Time Series](https://wesmckinney.com/book/time-series) chapter
of the book [Python for Data Analysis, 3rd Ed.](https://wesmckinney.com/book).
:::
...
Lag plots are scatter plots of the data shifted by a certain lag against the
original data.
...
Let's demonstrate a practical example of time series decomposition using
`statsmodels` routines `seasonal_decompose` and `STL`.
...
**Question**: What differences do you see between multiplicative and additive models?
...
The course notes have examples of both of these tests applied to the Air
Passengers dataset.
