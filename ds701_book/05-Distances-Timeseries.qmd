...
::: {.incremental}
* $d(x, x) = 0$
* $d(x, y) > 0 \forall x\neq y\qquad\qquad$ (positivity)
* $d(x, y) = d(y, x) \qquad\qquad~~$ (symmetry)
* $d(x, y)\leq d(x, z) + d(z, y) ~~$ (triangle inequality)
:::
:::
::: {.column width="30%"}
:::: {.fragment}
![](drawio/TriangleInequality.png){fig-align="center"}
::::
:::
::::
...
Let $\mathbf{u}, \mathbf{v}\in\mathbf{R}^{n}$ and $a\in\mathbb{R}$. The vector function $p(\mathbf{v})$ is called a __norm__ if

::: {.incremental}
* $p(a\mathbf{v}) = |a|p(\mathbf{v})$,
* $p(\mathbf{u} + \mathbf{v}) \leq p(\mathbf{u}) + p(\mathbf{v})$,
* $p(\mathbf{v}) = 0 \leftrightarrow \mathbf{v}$.
:::
