...
Recall we looked at an artificial neuron in the [Neural Networks I](23-NN-I-Intro.qmd) lecture.
...
The activation function is typically some nonlinear function that compresses the input in some way. Historically, it's been the sigmoid and $\tanh()$ functions. See for example [Hyperbolic Functions](https://en.wikipedia.org/wiki/Hyperbolic_functions#Tanh).

::: {.callout-important}
Without a nonlinear activation function, a neural network is just a linear model.
:::
...
![](figs/NN-figs/L24-fcn-dag.png){width="75%"}

From [Understanding Deep Learning](https://udlbook.github.io).
...
The example above shows a network with _3 inputs_, three hidden layers of 4, 2 and 3 neurons, respectively, followed by one layer that produces two continuous value outputs. This is regression model that is predicting two continuous valued
outputs.

E.g., a regression problem.
...
\mathbf{\hat{y}} &= \mathbf{\beta}_K + \mathbf{\Omega}_K \mathbf{h}_K \\
\end{aligned}
$$
...
So what we want to do is find the partial derivatives of $L$ with respect to each parameter in the network.

In other words, we want to compute

$$
\frac{\partial L}{\partial \mathbf{\beta}_k}, \frac{\partial L}{\partial \mathbf{\Omega}_k}, \frac{\partial L}{\partial \mathbf{\beta}_{k-1}}, \frac{\partial L}{\partial \mathbf{\Omega}_{k-1}}, \ldots, \frac{\partial L}{\partial \mathbf{\beta}_0}, \frac{\partial L}{\partial \mathbf{\Omega}_0}
$$

In the notation above, $\mathbf{\beta}_k$ and $\mathbf{\Omega}_k$ are the bias
vectors and weight matrices of the $k$-th layer. Just as with partial derivatives
with respect to scalars, there are well-defined rules for how to compute the
partial derivatives of vector and matrix expressions and, in fact, PyTorch and
TensorFlow implement these calculations.

So that we can update the parameters with the negative of the partial derivatives.

$$
\mathbf{\beta}_k \leftarrow \mathbf{\beta}_k - \frac{\partial L}{\partial \mathbf{\beta}_k}
$$

$$
\mathbf{\Omega}_k \leftarrow \mathbf{\Omega}_k - \frac{\partial L}{\partial \mathbf{\Omega}_k}
$$
...
![](figs/NN-figs/Train2ReLUDeriv.svg){width="50%"}
