...
An important tool for computing probabilities is provided by the following law.
...
When an experiment is performed, we might be interested in its actual outcome itself. However, more frequently we are interested in some function of the outcome. This leads us to the notion of a __random variable__.
...
Consider the follwing CDF of some random variable:
...
::: {.content-visible when-profile="web"}
As an exmpale, consider the single die, whose expected value is 3.5
:::
...
2. The covariance matrix is __positive semidefinite__, which means $\mathbf{x}^T \text{Cov}[X] \mathbf{x}\geq 0$ for all vectors $\mathbf{x}$.
...
Let $D$ have $n$ rows (i.e., $n$ observations).
...
::: {.content-visible when-profile="slides"}
## Low and High Variability continued
:::
...
$$F(a) = P(X \leq a) = 1 - e^{-\lambda x} \mbox{ for } a \geq 0.$$ 
...
However many other sums of random variables (not just Bernoulli trials) converge to the Gaussian as well.
:::
