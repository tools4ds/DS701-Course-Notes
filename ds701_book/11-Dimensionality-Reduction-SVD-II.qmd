...
- The similarity $p_{j\vert i}$ between points $i$ and $j$ is given by:
  $$
 ...
 - The t-SNE method uses $p_{ij} = \frac{p_{j\vert i} + p_{i\vert j}}{2d}$ where $d$ is the dimension of the point. This is because the probabilities $p_{j\vert i}$ and $p_{i\vert j}$ are different. Using this value of $p_{ij}$ is called symmetric t-SNE.
