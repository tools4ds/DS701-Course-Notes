...
A researcher is interested in how variables, such as _GRE_ (Graduate Record Exam
scores), _GPA_ (grade point average) and prestige of the undergraduate institution
...
Now, a linear function like $\alpha + \beta x$ cannot be used to predict
...
For any probabilty $p$, the __odds__ is defined as $p/(1-p)$, which is the ratio of the probability of an event to the probability of the non-event.
...
$\alpha + \beta x$ against $\log p/(1-p)$.
...
e^{\alpha + \beta x} &= \frac{p(x)}{1-p(x)} \quad \text{(exponentiate both sides)} \\
e^{\alpha + \beta x} (1-p(x)) &= p(x) \quad \text{(multiply both sides by $1-p(x)$)} \\
e^{\alpha + \beta x}  &= p(x) + p(x)e^{\alpha + \beta x} \quad \text{(distribute $p(x)$)} \\
...
This is a sigmoid function; when $\beta > 0$, $x\rightarrow \infty$, then
...
Another interpretation of $\alpha$ is that it gives the __base rate__ -- the
unconditional probability of a 1.   That is, if you knew nothing about a
particular data item, then $p(x) = 1/(1+e^{-\alpha})$.
...
coefficients $\alpha, \beta$ to fit:
...
We do the fitting by minimizing the sum of squared error ($\Vert\epsilon\Vert$).
...
then minimizing the sum of squared error is exactly the same as finding the
...
$\alpha$ and $\beta$ can be found by a __closed-form__ calculation.
...
$$
\text{Pr}(y_i \mid x_i) = 
\left\{\begin{array}{lr}\text{logit}^{-1}(\alpha + \beta x_i)& \text{if } y_i = 1\\
...
**Output** parameters $\widehat{\alpha}$ and $\widehat{\beta}$ that maximize the
...
The standard package will give us a correlation coefficient (a $\beta_i$) for each
independent variable (feature).

If we want to include a constant (ie, $\alpha$) we need to add a column of 1s (just
...
At the start of lecture I emphasized that logistic regression is concerned with
estimating a __probability__ model from __discrete__ (0/1) data. 
