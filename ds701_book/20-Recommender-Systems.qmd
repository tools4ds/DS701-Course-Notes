...
* $\alpha_u$ is the $n\times 1$ per-user offset of user $u$ and 
* $\beta_i$ is the $m\times 1$ per-item offset of item $i$.
...
Each row of $R$ represents an item and each column a user.
...
Now that we have leanred the biases, we can do a better job of estimating correlation:
...
where we have written out the vector L2 norm as the summation.
