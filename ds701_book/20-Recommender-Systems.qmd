...
Example: Let's look at a dataset for testing recommender systems consisting of Amazon movie reviews:
...
It looks like the `Id` column can be used as the dataframe index.
...
* $r_{ui}$ is the rating that 
* user $u$ gives to 
* item $i$, and 
* $S$ is the set of all ratings.
...
:::: {.fragment}
⟹ <span style="background-color: yellow;">Good for many users, fewer items</span>
::::
...
:::: {.fragment}
⟹ <span style="background-color: yellow;">Good for many items, fewer users</span>
::::
...
[^pearson]: See this part of the Course Notes for the definition of Pearson correlation coefficient.
...
Let's compute the Pearson correlation coefficient for the following two items:
...
* $\alpha_u$ is the per-user offset of user $u$ and 
* $\beta_i$ is the per-item offset of item $i$.
...
How can we estimate the $\alpha\textnormal{s}$, the $\beta\textnormal{s}$, and
the $\mu\textnormal{s}$?
...
:::: {.columns}
::: {.column width="50%"}
![](figs/L10-Movie-Latent-Space.png){width="100%" fig-align="center"}
...
* $R$ is $m\times n$, 
* $U$ is the $m\times k$ items matrix and 
* $V$ is the $n\times k$ users matrix.
...
![](figs/L20-netflix-1.png){width="70%" fig-align="center"}
...
* [How YouTube Leads Privacy-Seeking Users Away from Reliable Information,](http://www.cs.bu.edu/faculty/crovella/paper-archive/youtube-fairumap20.pdf) 
   Larissa Spinelli and Mark Crovella, _Proceedings of the Workshop on Fairness in User Modeling, Adaptation, and Personalization (FairUMAP)_, 2020.   
...
* [Fighting Fire with Fire: Using Antidote Data to Improve Polarization and Fairness of Recommender Systems,](http://www.cs.bu.edu/faculty/crovella/paper-archive/wsdm19-antidote-data.pdf)
  Bashir Rastegarpanah, Krishna P. Gummadi and Mark Crovella _Proceedings of WSDM_, 2019.
...
* Comparison of CF and MF, highlighting their strengths and weaknesses.
* Overview of advanced techniques used by the Netflix prize-winning team to improve MF accuracy.
