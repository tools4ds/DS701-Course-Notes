...
The following is an outline of the procedure for performing PCA and determining the principal components. 

After this outline, we will present how to actually compute the principal components.
...
Recalling the outline of our PCA steps, we see that steps 2-4 are accomplished by computing the eigen-decomposition of the covariance matrix $S = \frac{1}{m-1}X^T X = V\Lambda V^{T}$.

Step 5 is accomplished by computing $XV$.
...
You can then obtain the principal components from the right singular vectors $V$. The projected data into the lower dimensional space is $XV$. The eigenvalues of $S$ are obtained from squaring the entries of $\Sigma$ and scaling them by $\frac{1}{m-1}$
